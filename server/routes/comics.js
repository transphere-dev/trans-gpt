...
router.post('/ocr', (req, res) => {
    ...
    try {
     ...
     const extract  = cmd.runSync(`python ./server/ocr/ocr.py --userId ${userId} --cname ${comicName} `)
...
router.get('/:userId', (req, res) => {
    ...
    try {
        let comics = []
        // Fertch the comics
        const directory = path.join(path.dirname(__dirname), 'uploads/comics', `${userId}`);
