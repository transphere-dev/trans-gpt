    ...
    const verifyEmail = async () => {
      ...
      try {
        const response = await fetch(
          `http://${process.env.NEXT_PUBLIC_SERVER_URL}:${process.env.NEXT_PUBLIC_PORT}/api/users/verify/${token}`
        );
        if (response.ok) {
      ...
      {verificationStatus === "error" && (
        <>
          ...
          <Text mt={"2%"} align={"center"}>
            The verification link may be invalid or expired. Log into your
